...
let a = localStorage.getItem("cli");
if (!a) {
  window.location.href = "login";
}
document.querySelector(".sc").innerHTML = `code: ${a}`;
...
const DBScelto = ref(database, `Impegni/${a}`);
...
function creaHTML() {
  ...
  if (i <= 0) {
    ...
    document.querySelector(".agg").classList.add("hi");
  } else {
    document.querySelector(".arsx").classList.remove("hi");
    document.querySelector(".agg").classList.remove("hi");
  }
  boxgiorni.innerHTML = "";
...
document.addEventListener("click", (e) => {
  if (action) {
    ...
    if (e.target.classList[1] == "arsx" && i > 0) {
      ...
      creaHTML();
      action = true;
    } else if (e.target.classList.contains("agg")) {
      localStorage.setItem("dc", 0);
      location.reload();
    } else if (
      ...
      e.target.parentElement.classList.contains("info")
    ) {
      action = true;
      document.querySelector(".sc").classList.add("active");
      setTimeout(function () {
        document.querySelector(".sc").classList.remove("active");
      }, 5000);
...
// let mobile;
// if (
//   navigator.userAgent.split("(")[1].split(" ")[0].split(";")[0] == "Windows"
// ) {
//   mobile = false;
// } else {
//   mobile = true;
